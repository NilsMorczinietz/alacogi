 ...
 */
export const authenticateParamToken = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (!token) {
    return res
      .status(401)
      .json({ message: "Kein Token als Parameter übergeben" });
  }

  if (!VALID_TOKENS.includes(token)) {
    return res
      .status(403)
      .json({ message: "Zugriff verweigert – ungültiger Parameter-Token" });
  }
...
export const combineAuth = (
  ...
  return res
    .status(403)
    .json({ message: "Zugriff verweigert – kein gültiger Token gefunden" });
};
